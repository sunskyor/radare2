...
repeat:
		...
		case R_ANAL_OP_TYPE_MOV:
			...
			&& !strcmp (op->src[0]->reg->name, op->dst->reg->name)) {
				skip_ret = skip_hp (anal, fcn, op, bb, addr, NULL, oplen, delay.un_idx, &idx);
				if (skip_ret == 1) {
					goto repeat;
				}
				if (skip_ret == 2) {
		...
		case R_ANAL_OP_TYPE_LEA:
			...
			if (anal->opt.hpskip && regs_exist (op->src[0], op->dst)
			&& !strcmp (op->src[0]->reg->name, op->dst->reg->name)) {
				skip_ret = skip_hp (anal, fcn, op, bb, at, NULL, oplen, delay.un_idx, &idx);
				if (skip_ret == 1) {
					goto repeat;
				}
				if (skip_ret == 2) {
				...
				if (is_delta_pointer_table (anal, fcn, op->addr, op->ptr, &jmptbl_addr, &casetbl_addr, jmp_aop)) {
					...
					if (try_get_jmptbl_info (anal, fcn, addr, bb, &table_size, &default_case)
						...
						ret = casetbl_addr == op->ptr
							? try_walkthrough_jmptbl (anal, fcn, bb, depth - 1, addr, jmptbl_addr, op->ptr, 4, table_size, default_case, 4)
							: try_walkthrough_casetbl (anal, fcn, bb, depth - 1, addr, jmptbl_addr, casetbl_addr, op->ptr, 4, table_size, default_case, 4);
						if (ret) {
							lea_jmptbl_ip = addr;
						}
					}
				...
				if (op->ptr != UT64_MAX) {
					...
					if (cmpval != UT64_MAX && default_case != UT64_MAX && (op->reg || op->ireg)) {
						if (op->ireg) {
							...
							ret = walkthrough_arm_jmptbl_style (anal, fcn, bb, depth - 1, op->addr, op->ptr, anal->bits >> 3, table_size, default_case, ret);
						}
